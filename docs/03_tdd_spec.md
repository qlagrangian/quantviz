...
| AAD-07 | ランダム式（seed 固定、3 入力に 10 種の演算を 20 段合成 × 100 式）で中心差分と一致: 各比較は 1e-8·\|fd\| + 32·eps·\|f\|/h、\|fd\| > 0.1 の部分集合で相対 1e-8（200 seed で最悪 2.6e-9、差分自体の丸め床を上回る余裕 3.9×） | N | ✅ |
| AAD-08 | `reserve` 後の記録・逆伝播でアロケーションなし（glibc の `mallinfo2` をヒープ計として自己検証してから計測。非 glibc / ASan では構造検査だけ）。容量超過でも割り当て・クラッシュなし | U | ✅ |
