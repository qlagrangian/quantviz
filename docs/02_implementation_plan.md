...
| **M4** | 動的処理 | LSM, Almgren–Chriss, Merton HJB, パス束・執行軌道・価値関数面 | パス並列、DP のメモリ設計 | 最適停止、執行のリスク回避、HJB | ✅ 完了 |
...
**実績（完了時点）** 10 シーン（+ LSM American, Optimal execution, Merton HJB）。テスト 207 ケース（M3 の 179 + 28）、ASan/UBSan/TSan/FMA レッグ Green、`-Werror`。LSM は N = 20000・Laguerre 4 で CN 参照 6.0848 に対し 6.0648 ± 0.030（z = −0.66）、AC は Almgren–Chriss (2000) Table 1 を桁まで再現し MC と 1 SE 以内、HJB は π* の誤差 1.5e-5（内側 90 %）で空間 2 次・時間 1 次収束。レビューで直した実装上の穴: AC の sinh 比が κT ≳ 700 で inf/inf = NaN（e^{a−b}(1−e^{−2a})/(1−e^{−2b}) 形に）、HJB の制御を [0, 5] にクランプする一方で境界・閉形式の κ が無制約のままで、スライダー到達域の約半分で面が非単調・非有限（κ を同じ π_c から作る制約付き閉形式に）、LSM の `gbm_step` が FMA レッグでインライン先ごとに縮約差を出す（`FP_FAST_FMA` で `std::fma` を明示）。計画からの逸脱: **LSM-02 / LSM-04 は 95 % CI ではなく §2.4 の 4 SE**（95 % CI 検定は構造的に 5 % の seed で落ち、CI の 3 OS は正規乱数列が違う。基底は Laguerre 3 → 4 でバイアス −0.026 → −0.005）、LSM シーンは「直近に回帰できた時点のフィットを保持」（ATM 既定では最終時点に ITM パスが無い）、執行シーンの λ 梯子は現在 λ の周りに動きフロンティア格子は固定、ACSCENE-03（板との結線）は未実施、HJB シーンの 3D 高さは符号付き log。並列の viewer 検証で XTest のクリックが別エージェントの窓に落ちる問題は `drive.py` の `QV_SEND=1`（XSendEvent で窓を指定）で解消。
