...
M0–M3 are complete (7 scenes: Streaming, Greeks, GARCH, Kalman pair, Vol surface, FDM American, Order book). M4 (LSM / Almgren–Chriss / Merton HJB) and M5 (tape AAD, performance panel) are planned in `02_implementation_plan.md`; per-milestone execution plans with type contracts live in `docs/superpowers/plans/`. The module tables in `01_design.md` §6 list files and APIs.
...
- **Running the viewer**: use the project skill `run-viewer` (`.claude/skills/run-viewer/drive.py`: launch, screenshot by window id, click/drag/wheel/key). Pin your window with `QV_WID` / `QV_PID_FILE` when several viewers share the display. Every scene must pass the manual checklist (snapshots/s ≈ steps_per_second, dropped 0, Pause → Step ×5 = seq +5, Reset clears plots, paused sliders update on the next frame).
- **Parallel work**: tasks run in isolated `git worktree`s with per-task build directories; shared files (`main.cpp`, CMake lists, `docs/*`, `README.md`, CI) are touched only by the integrator. Test names must be ASCII (ctest passes them on the Windows command line; CI enforces it).
