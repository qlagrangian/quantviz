...
M0 (Streaming scene) is complete. M1–M5 (Greeks/GARCH/Kalman, 3D FDM, order book, LSM/HJB, AAD) are planned in `02_implementation_plan.md`; the module tables in `01_design.md` §6 list the intended files and APIs.
...
- **Snapshot rules**: fixed-size POD, no pointers/`std::vector`/`std::string` (use `std::array`), target ≤ 128 bytes, must carry `seq` (0 = never stepped or just reset; panels skip it) and any true parameter values the panel needs for reference lines. Large state (grids, order books) is reduced before it goes in; a triple buffer is planned for grid-sized state in M2.
...
- **UI → Command**: ImGui sliders are `float`; widen to `double` when building the `Command`. Send only on the frame the widget changed; a failed `send` (full ring) can be ignored. Reset sends `Command::reset()` and clears the viewer's `History` in the same frame.
